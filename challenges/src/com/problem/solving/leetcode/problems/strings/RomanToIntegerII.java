 ...
 *
 * <p>The same principle applies to the number nine, which is written as IX. There are six instances
...
public class RomanToIntegerII {
  ...
  private final Map<Character, Integer> romanToIntValue = Map.of(
          'I', 1,
          'V', 5,
          'X', 10,
          'L', 50,
  ...
  public int romanToInt(String s) {
    ...
    for (int i = 0; i < numberSize; i++) {
      int currentValue = romanToIntValue.get(s.charAt(i));

      if ((i + 1) < numberSize && currentValue < romanToIntValue.get(s.charAt(i + 1))) {
        int nextValue = romanToIntValue.get(s.charAt(i + 1));
        romanToInt    = romanToInt + (nextValue - currentValue);
        i++;
        continue;
      }

      romanToInt = romanToInt + currentValue;
    }
